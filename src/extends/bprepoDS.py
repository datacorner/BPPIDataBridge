...
from extends.bprepo.blueprismSQLBuilder import blueprismSQLBuilder
from extends.bprepo.bpLogsProcessing import bpLogsProcessing
import extends.constants as C
...
class bprepoDS(odbcDS):
    ...
    def parametersValidationFile(self):
        filename = self.getResourceFile(P.RESOURCE_PKGFOLDER_DATASOURCES, CFGFILES_DSOBJECT)
        return str(filename)

    @property
    def getQueryFile(self):
        return importlib.resources.files("config").joinpath(C.BPLOG_INI4SQL)
